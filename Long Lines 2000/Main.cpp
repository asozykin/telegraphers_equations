...
{
   Application->MessageBox("Рассчет по данным параметрам невозможен из-за ограниченности возможностей компьютера. Попробуйте сначала изменить длину линии.","Параметры заданы некорректно ",MB_OK | MB_ICONWARNING);
   writeToEdits();
}
catch(EOverflow&)
{
...
//    }; // End of RLC input
    ...
    if((zng1 != zw1)&&(zng3 != zw3))
    {
        ...
        {
           Application->MessageBox("Рассчет по данным параметрам невозможен из-за ограниченности возможностей компьютера ( произведение gamma*ll, где ll - длина линии, очень велико ). Попробуйте сначала изменить длину линии.","Параметры заданы некорректно ",MB_OK | MB_ICONWARNING);
           writeToEdits();
           drawChart();
        };
    }
